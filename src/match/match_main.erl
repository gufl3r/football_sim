            ...
            case Match#match.status of
                in_progress ->
                    io:format("Day: ~p, match ~p: ~s ~p vs ~p ~s ~p:00~n",
                        [Match#match.day, Match#match.match_id,
                         Match#match.team1#team.acronym, Match#match.goals_team1,
                         Match#match.goals_team2, Match#match.team2#team.acronym,
...
advance_status(Match) ->
    case Match#match.status of
        ...
        finished ->
            io:format("Match ~p is already finished.~n", [Match#match.match_id]),
            Match;
        _ ->
            io:format("Fatal error: match ~p has invalid status~n", [Match#match.match_id]),
            Match
    end.
